...
functionality, is implemented as a plugin: L{checkhtml}.

Entry Point
===========

L{main} parses command line arguments and then calls L{run} to start
a test run.

A test run starts by creating a L{Spider} to crawl the web
app/site under test, a L{Scribe} to collect the reporting
and a L{PageChecker} to load and check the pages.
