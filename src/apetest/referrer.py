...
L{LinkSet} models links such as C{<a href=} HTML tags.
L{Redirect} models a redirection from one URL to another.
L{Form} models a form that can generate requests for other documents
when submitted.
"""
...
class Referrer:
    ...
    def __init__(self, page_url: str):
        """
        Initialize the referrer's L{page_url} from the argument and
        sets the L{method} to C{'get'}.
        """
    ...
    def has_request(self, request: Request) -> bool:
        """
        Returns C{True} iff this referrer could generate C{request}.

        Note that it is possible for this function to return C{True}
    ...
    def iter_requests(self) -> Iterator[Request]:
        ...
        the test coverage when we only have time to try the first M{N} requests.
...
class Redirect(Referrer):
    ...
    def __init__(self, target: Request):
        """Initialize a redirection to C{target}."""
        Referrer.__init__(self, target.page_url)

        self.target = target
        """The L{Request} that a user agent is redirected to."""
...
class LinkSet(Referrer):
    ...
    def add(self, request: Request) -> None:
        """
        Adds a linked L{Request} to this set.
...
class Form(Referrer):
    ...
    def __init__(self, page_url: str, method: str, controls: Collection[Control]):
        """
        Initialize form.

        @param page_url:
            URL at which this form is submitted (C{action} HTML attribute).
        @param method:
            C{'get'} or C{'post'}.
        @param controls:
        ...
        """
        Collection of L{Control} objects that represent user interface
        input elements that contribute to the submitted query.
        """
    ...
    def has_request(self, request: Request) -> bool:
        ...
        class MatrixIterator(Iterator[T]):
            ...
            def remove_control(self, control_index: int) -> None:
                del produces_pairs[control_index]
                del produces_empty[control_index]
                self._changed = True

            def remove_pair(self, pair_index: int) -> None:
        ...
        class ControlIterator(MatrixIterator[tuple[list[bool], bool]]):
            ...
            def remove_control(  # type: ignore[override]
        ...
        class PairIterator(MatrixIterator[Sequence[int]]):
            ...
            def remove_pair(  # type: ignore[override]
                # pylint: disable=arguments-differ
                self,
            ) -> None:
            ...
            def _value_at(self, index: int) -> Sequence[int]:
                return [
                    controlIndex
        ...
        def solve_recursive(control_index: int) -> bool:
    ...
    def iter_requests(self) -> Iterator[Request]:
        ...
        while True:
            index = progress
            query = list(self.base_query)
            for alternatives in self.all_alternatives:
                index, choice = divmod(index, len(alternatives))
                name, value = alternatives[choice]
                if name is not None:
                    assert value is not None
                    query.append((name, value))
            yield Request(self.page_url, query, maybe_bad=True)
